#!/usr/bin/env python3
"""
TechHorizon 最终版数据收集器模块 - 移除Gitee数据源
"""
    ...
    def collect(self, limit: int = 35) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for story_id in top_stories[:limit]:
                ...
                if story and story.get('score', 0) > 5:  # 降低分数阈值以获取更多数据
                    event = {
...
def collect_all_sources() -> List[Dict[str, Any]]:
    ...
    # 配置每个数据源的收集数量（调整后总计约145条/天）
    source_limits = {
        'github_trending': 30,      # +5
        'hacker_news': 35,          # +5  
        'readhub': 25,              # +5
        'oschina': 20,              # +5
        'juejin': 20,               # +5 (需要修复API)
